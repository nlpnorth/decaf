...
def build_gendered_filters(gender):
	...
	filters[('subj', f'pred')] = Filter(
		criteria=[
			Criterion(
				operation='AND',
				conditions=[
					Condition(stype='upos', values=['PRON']),
					Condition(stype='deprel', values=['nsubj']),
					Condition(stype='Case', values=['Nom']),
					Condition(stype='Number', values=['Sing']),
					Condition(stype='Person', values=['3']),
					Condition(stype='Gender', values=[gender])
				]
			),
			Criterion(
				operation='AND',
				conditions=[
					Condition(stype='upos', values=['VERB', 'AUX']),
					Condition(stype='deprel', values=['root', 'conj', 'advcl', 'aux'])
				]
			)
		],
		sequential=True,
	...
	# type 1
	# "The", "farmer", "gave", "the", "teacher", "a", "bag", "of", "eggs", ["because", "she", "was"], "grateful", "."
	filters[('conj', 'subj', f'pred')] = Filter(
		criteria=[
			Criterion(
				operation='AND',
				conditions=[
					Condition(stype='upos', values=['SCONJ'], literals=['and', 'as', 'because', 'but']),
					Condition(stype='deprel', values=['mark'])
				]
			)
		] + filters[('subj', f'pred')].criteria,
		sequential=True,
		hierarchy=['sentence', 'token']
	)
	...
	filters[('pred', f'obj')] = Filter(
		criteria=[
			Criterion(
				operation='AND',
				conditions=[
					Condition(stype='upos', values=['VERB']),
					Condition(stype='deprel', values=['root', 'conj'])
				]
			),
			Criterion(
				operation='AND',
				conditions=[
					Condition(stype='upos', values=['PRON']),
					Condition(stype='deprel', values=['iobj']),
					Condition(stype='Case', values=['Acc']),
					Condition(stype='Number', values=['Sing']),
					Condition(stype='Person', values=['3']),
					Condition(stype='Gender', values=[gender])
				]
	...
	filters[('adp', f'pron')] = Filter(
		criteria=[
			Criterion(
				conditions=[
					Condition(stype='upos', values=['ADP'])
				]
			),
			Criterion(
				operation='AND',
				conditions=[
					Condition(stype='upos', values=['PRON']),
					Condition(stype='deprel', values=['obl', 'nmod']),
					Condition(stype='Case', values=['Acc', 'Gen']),
					Condition(stype='Number', values=['Sing']),
					Condition(stype='Person', values=['3']),
					Condition(stype='Gender', values=[gender])
				]
...
def build_content_filters():
	...
	# filters[('det', 'subj', 'pred', 'det', 'obj')] = Filter(
	# 	criteria=[
	# 		Criterion(
	# 			operation='AND',
	# 			conditions=[
	# 				Condition(stype='upos', values=['DET']),
	# 				Condition(stype='Definite', values=['Def'])
	# 			]
	# 		),
	# 		Criterion(
	# 			operation='AND',
	...
	# 			conditions=[
	# 				Condition(stype='upos', values=['NOUN']),
	# 				Condition(stype='deprel', values=['obj'])
	# 			]
	# 		),
	# 	],
	# 	sequential=True,
...
def order_by_specificity(content_ranges, positive_ranges, negative_ranges):
	...
	# single-gender + trigram + occupation
	single_occupation_ranges = append_and_reduce(
		[],
		positive_ranges[('conj', 'subj', 'pred')] - negative_ranges[('pron',)] & content_ranges['occupations']
	)
	# single-gender + bigram + occupation
	single_occupation_ranges = append_and_reduce(
		single_occupation_ranges,
		positive_ranges[('subj', 'pred')] - negative_ranges[('pron',)] & content_ranges['occupations']
	)
	single_occupation_ranges = append_and_reduce(
		single_occupation_ranges,
		positive_ranges[('pred', 'obj')] - negative_ranges[('pron',)] & content_ranges['occupations']
	)
	single_occupation_ranges = append_and_reduce(
		single_occupation_ranges,
		positive_ranges[('adp', 'pron')] - negative_ranges[('pron',)] & content_ranges['occupations']
	)
	...
	# add specificity level to results
	ordered_ranges = append_and_reduce(
		ordered_ranges,
		set.union(*single_occupation_ranges)
	)

	# single-gender + trigram
	single_ranges = append_and_reduce(
		list(ordered_ranges),
		positive_ranges[('conj', 'subj', 'pred')] - negative_ranges[('pron',)]
	)
	# single-gender + bigram
	single_ranges = append_and_reduce(
		single_ranges,
		positive_ranges[('subj', 'pred')] - negative_ranges[('pron',)]
	)
	single_ranges = append_and_reduce(
		single_ranges,
		positive_ranges[('pred', 'obj')] - negative_ranges[('pron',)]
	)
	single_ranges = append_and_reduce(
		single_ranges,
		positive_ranges[('adp', 'pron')] - negative_ranges[('pron',)]
	)
	...
	ordered_ranges = append_and_reduce(
		ordered_ranges,
		set.union(*single_ranges)
	)

	# mixed-gender + trigram + occupation
	mixed_occupation_ranges = append_and_reduce(
		list(ordered_ranges),
		positive_ranges[('conj', 'subj', 'pred')] & content_ranges['occupations']
	)
	# mixed-gender + bigram + occupation
	mixed_occupation_ranges = append_and_reduce(
		mixed_occupation_ranges,
		positive_ranges[('subj', 'pred')] & content_ranges['occupations']
	)
	mixed_occupation_ranges = append_and_reduce(
		mixed_occupation_ranges,
		positive_ranges[('pred', 'obj')] & content_ranges['occupations']
	)
	mixed_occupation_ranges = append_and_reduce(
		mixed_occupation_ranges,
		positive_ranges[('adp', 'pron')] & content_ranges['occupations']
	)
	...
	ordered_ranges = append_and_reduce(
		ordered_ranges,
		set.union(*mixed_occupation_ranges)
	)

	# mixed-gender + trigram
	mixed_ranges = append_and_reduce(
		list(ordered_ranges),
		positive_ranges[('conj', 'subj', 'pred')]
	)
	# mixed-gender + bigram
	mixed_ranges = append_and_reduce(
		mixed_ranges,
		positive_ranges[('subj', 'pred')]
	)
	mixed_ranges = append_and_reduce(
		mixed_ranges,
		positive_ranges[('pred', 'obj')]
	)
	mixed_ranges = append_and_reduce(
		mixed_ranges,
		positive_ranges[('adp', 'pron')]
	)
